...
int jhcGraphizer::add_tag (int kind, const char *alist) const
{
  jhcAliaAttn *attn = &(core->attn);
  jhcAliaChain *ch, *add;
  jhcAliaDir *dir;
...
int jhcGraphizer::attn_tag (const char *alist) const
{
  jhcAliaAttn *attn = &(core->attn);
...
int jhcGraphizer::HarvestLex (const char *kern) 
{
  char fname[200];
...
int jhcGraphizer::scan_lex (const char *fname)
{
...
void jhcGraphizer::save_word (char (*list)[40], int& cnt, const char *term) const
{
  int i;
...
int jhcGraphizer::gram_cats (const char *fname, const char *label) const
{
  FILE *out;
