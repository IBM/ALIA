...
public:
  ...
  double NeckIdle (UL32 now) const  
    {return(0.001 * jms_diff(now, ntime));}
  double LiftIdle (UL32 now) const  
    {return(0.001 * jms_diff(now, ltime));}
  double ArmIdle (UL32 now) const   
    {return(0.001 * jms_diff(now, atime));}
  double GripIdle (UL32 now) const  
    {return(0.001 * jms_diff(now, gtime));}
  double ManipIdle (UL32 now) const 
    {return(0.001 * __min(jms_diff(now, atime), jms_diff(now, gtime)));}
  double TurnIdle (UL32 now) const  
    {return(0.001 * jms_diff(now, ttime));}
  double MoveIdle (UL32 now) const  
    {return(0.001 * jms_diff(now, mtime));}
  double BaseIdle (UL32 now) const  
    {return(0.001 * __min(jms_diff(now, ttime), jms_diff(now, mtime)));}
